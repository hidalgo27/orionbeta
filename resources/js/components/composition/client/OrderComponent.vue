                ...
                <div class="row">
                    <div class="col-md-12">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="index.html">Homed</a></li>
                        ...
                        <div class="dashboard-right">
                            <div class="row">
                                ...
                                <div class="col-lg-12 col-md-12">
                                    <div class="pdpt-bg" v-for="(order, key) in order_p" :key="key">
                                        <div class="pdpt-title">
                                            <h6>Se entrega: {{ order.fecha_entrega }}, {{ order.hora_entrega }}</h6>
                                        </div>
                                        <div class="order-body10">
                                            <ul class="order-dtsll" v-for="productos_or in order.productos_ordenados">
                                                <li>
...
import orderBy from "lodash/orderBy";
export default {
    components: { HeaderComponent, TopComponent, HeaderClientComponent, MenuClientComponent},
    props: ["user"],
    setup(props){
        const products = ref([]);
        ...
        const cartState = reactive({
            ...
            order_p : computed(() => {
                return orderBy(orders.value.filter(order => order.state > '0'), 'created_at', 'desc')
            }),

            // sumTotal : computed(() => {
            //     // const summation = filteredPass();
            //     return (orders.value.filter(order => order.id == '13')).reduce(
            //         (sum, cur) => {
            //             console.log(cur.total, cur.tax)
            //             const prevPrice = sum.total || sum;
            //             const tax = sum.tax || sum;
            //             return parseFloat(cur.total) + parseFloat(cur.tax)
            //         }, 0
            //     )
            // }),
        });
        ...
        // function rowTotal(base) {
        //     return orders.value.reduce( (sum,cur) => sum+cur[base] , 0);
        // }
        //
        // function filteredPass() {
        //     return orders.value.filter( x => x.id == "14");
        // }
        ...
        fetch("http://sistemaorion.nebulaperu.com/api/v1/orders/"+props.user.id+"/get-all-client")
            .then(res => res.json())
            .then(data => {
                orders.value = data;
            });
